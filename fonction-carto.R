...
#CARTOGRAPHIE-----
# Classes de pH
breaks <- c(-Inf, 4, 5,6, 6.5, 7, 8, 9, Inf)

labels <- c("[3 - 4[", "[4 – 5[", "[5 – 6[", "[6 – 6.5[", "[6.5 – 7[", "[7 – 8[",

          "[8 – 9[",  "[9 - 10]")

# Appel fonction 
r <- c("predKEDINLA_centroides_final.tif")
noms <- c("KED_INLA_cent")
...
pH_med <- pH_med %>%
  mutate(pH_class = factor(cut(moy_pH, breaks = breaks, labels = labels, include.lowest = TRUE),
                           levels = labels))
