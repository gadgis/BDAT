run_inla_spde_core <- function(dataINLA, data_test, name,
  ...
  eval <- Myeval(fitted_df$pred, data_test[[name]]) %>%
    mutate(method = paste0("INLA", type, "_", substr(approach, 1, 1), substr(type_val, 1, 1)))
  
  return(list(preds = fitted_df$pred, evaluation = eval, detail = fitted_df))
}