  ...
  resu_rep <- foreach(rep = 1:repets, .combine = rbind,
                      ...
                      .export = c("Myeval","matern","preds_RF"))%dopar% {
                        ...
                        for (i in 1:k) {
                          ...
                          cmp <- elt ~ Intercept(1) + rfpred(predRF, model = "linear") + field(coordinates, model = matern)
                          ...
                          data_sample$predINLAKED[mask] <- preds[mask]
                        }
                        
                        Myeval(data_sample$predINLAKED, data_sample[[name]]) %>%
...
saveRDS(results_summary, "resultats_INLAKED_Degrad.rds")